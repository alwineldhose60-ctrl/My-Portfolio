"use client";

import { motion, useScroll, useTransform } from "framer-motion";
import { useRef } from "react";
import Image from "next/image";
import Parallax from "../animations/Parallax";
import FadeIn from "../animations/FadeIn";
import Magnetic from "../animations/Magnetic";
import Link from "next/link";
import { ArrowDown } from "lucide-react";

export default function Hero() {
  const ref = useRef<HTMLDivElement>(null);
  const { scrollYProgress } = useScroll({
    target: ref,
    offset: ["start start", "end start"],
  });

  const backgroundY = useTransform(scrollYProgress, [0, 1], ["0%", "50%"]);
  const textY = useTransform(scrollYProgress, [0, 1], ["0%", "15%"]);
  const imageScale = useTransform(scrollYProgress, [0, 1], [1, 1.1]);
  const opacity = useTransform(scrollYProgress, [0, 0.8], [1, 0]);

  return (
    <section
      ref={ref}
      className="relative w-full h-screen flex flex-col items-center justify-center overflow-hidden bg-[#050505]"
    >
      {/* 1. Background Layer */}
      <motion.div style={{ y: backgroundY }} className="absolute inset-0 z-0">
        <div className="absolute inset-0 opacity-20 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-gray-700 via-gray-900 to-black"></div>
      </motion.div>

      {/* 2. Central Image Layer (Fills Section) */}
      <div className="absolute inset-0 flex items-center justify-center z-10 p-4 py-24 md:p-12 lg:p-20">
        <FadeIn delay={0.2} direction="up" className="relative w-full h-full">
          <motion.div
            style={{ scale: imageScale }}
            className="relative w-full h-full rounded-2xl md:rounded-3xl overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.8)] border border-white/60"
          >
            <Image
              src="/images/1000132537.jpg.jpeg"
              alt="Alwin Eldhose"
              fill
              className="object-cover hidden md:block"
              priority
            />

            <Image
              src="/images/1757865924771.jpg.jpeg"
              alt="Alwin Eldhose"
              fill
              className="object-cover md:hidden"
              priority
            />
            {/* Darker Overlay for Text Contrast */}
            <div className="absolute inset-0 bg-black/30 h-full"></div>
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent"></div>

            <div className="absolute bottom-10 left-10 md:bottom-20 md:left-20">
              <p className="text-sm md:text-base text-gray-300 mb-2 font-mono uppercase tracking-[0.2em] text-opacity-80">
                Alwin Eldhose
              </p>
              <h2 className="text-3xl md:text-5xl font-bold text-white tracking-tight">
                Full-Stack Developer
              </h2>
            </div>
          </motion.div>
        </FadeIn>
      </div>

      {/* 3. Typography Layer (Top-most) */}
      {/* We use mix-blend-color for the grayscale effect on the image area */}
      <div className="absolute inset-0 z-30 pointer-events-none flex flex-col justify-between p-6 md:p-12 lg:p-20">
        {/* Top Titles */}
        <div className="flex justify-end items-start w-full pe-4 md:pe-12">
          <motion.div
            style={{ opacity }}
            className="text-right max-w-[150px] md:max-w-[300px] pt-24 md:pt-8"
          >
            <p className="text-gray-300 text-sm md:text-2xl leading-[1.1] font-medium tracking-tight uppercase opacity-60">
              Build
              <br />
              Interactive <br />
              Modern <br /> Scalable
              <br /> UI's
            </p>
          </motion.div>
        </div>
      </div>

      {/* 4. Floating Slash Labels (Top-most) */}
      <div className="absolute inset-0 pointer-events-none z-40 hidden md:block">
        <div className="absolute top-[40%] left-[5%] lg:left-[12%]">
          <Parallax offset={-40}>
            <p className="font-semibold text-xs lg:text-sm text-white/50 flex items-center gap-2 tracking-widest uppercase">
              <span className="text-white/20">/</span> Website Design.
            </p>
          </Parallax>
        </div>
        <div className="absolute top-[60%] left-[8%] lg:left-[18%]">
          <Parallax offset={30}>
            <p className="font-semibold text-xs lg:text-sm text-white/50 flex items-center gap-2 tracking-widest uppercase">
              <span className="text-white/20">/</span> AI Solutions.
            </p>
          </Parallax>
        </div>

        <div className="absolute top-[45%] right-[5%] lg:right-[12%]">
          <Parallax offset={50}>
            <p className="font-semibold text-xs lg:text-sm text-white/50 flex items-center gap-2 tracking-widest uppercase text-right">
              <span className="text-white/20">/</span> UI Architecture.
            </p>
          </Parallax>
        </div>
        <div className="absolute top-[65%] right-[8%] lg:right-[18%]">
          <Parallax offset={-25}>
            <p className="font-semibold text-xs lg:text-sm text-white/50 flex items-center gap-2 tracking-widest uppercase text-right">
              <span className="text-white/20">/</span> Next.js Expert.
            </p>
          </Parallax>
        </div>
      </div>
    </section>
  );
}
